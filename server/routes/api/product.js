...
router.get("/item/:slug", async (req, res) => {
  try {
    ...
    const productDoc = await Product.findOne({ slug, isActive: true })
      ...
      .populate("merchant", "_id name email business phoneNumber brandName");
...
router.get("/list", async (req, res) => {
  try {
    ...
    const basicQuery = getStoreProductsQuery(min, max, tag, rating);
    ...
    async function getMerchantDetailsById(merchantId) {
      const merchants = await Merchant.findById(merchantId);
      return merchants;
    }
    ...
    const productsWithMerchants = await Promise.all(
      products.map(async (product) => {
        if (product.merchant) {
          ...
          return { ...product, merchantDetails };
        }
        return product;
      })
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const merchant = req.body.merchant._id;
    ...
    try {
      ...
      if (req.user.merchant) {
        const brands = await Brand.find({
          ...
          "merchant",
          "_id name email business phoneNumber brandName"
        );
        ...
        products = await Product.find({})
          ...
          .populate("merchant", "name email business phoneNumber brandName");
      }
      res.status(200).json({
        products,
      });
